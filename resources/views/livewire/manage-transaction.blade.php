            ...
            <tbody>
                @forelse($transactions as $transaction)
                    <tr class="transition-all hover:bg-gray-50">
                        ...
                        <td class="p-3">{{ $transaction->forme->designation ?? '-' }}</td>
                        <td class="p-3">{{ $transaction->type->code ?? '-' }}</td>
                        <td class="p-3">{{ $transaction->volume }}</td>
                        <td class="p-3">
                            <div class="btn-group" role="group">
                                <button wire:click="showDetails({{ $transaction->id }})" class="btn btn-sm btn-primary" title="Voir détails">
                                    <i class="fas fa-eye"></i>
                                </button>
                                <a href="#" {{-- <a href="{{ route('admin.transaction.edit', $transaction) }}" --}} class="mr-2 btn btn-sm btn-primary me-2"
                                    title="Éditer">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <button wire:click="confirmDelete({{ $transaction->id }})" class="btn btn-sm btn-danger" title="Supprimer">
